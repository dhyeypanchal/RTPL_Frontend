...
function OfficeDetails() {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await instance.get("/api/company/office/getOfficelist");
  ...
  useEffect(() => {
    const filterData = () => {
      const filter = data.filter((data) =>
        data.id.toString().toLowerCase().includes(searchQuery.toLowerCase()) || data.CompanyId.toString().toLowerCase().includes(searchQuery.toLowerCase())
      );
  ...
  const deleteOfficeData = async (cell) => {
    const token = localStorage.getItem("token")

    // axios request
    try {
      const req = await instance.delete(`/api/company/office/${cell.id}`, {
        headers: {
  ...
  const saveEdit = async (row) => {
    ...
    try {
      const req = await instance.put(`/api/company/office/${row.id}`,
        {
  ...
  const columns = [
    {
      name: <span style={{ fontSize: "125%" }}><i className="fa-solid fa-calendar-days" style={{ marginRight: "4px" }} />Office Id</span>,
      selector: (row) => row.id,
    ...
    {
      name: <span style={{ fontSize: "125%" }}><i className="fa-solid fa-list" style={{ marginRight: "4px" }} />Company Id</span>,
    ...
    {
      name: <span style={{ fontSize: "125%" }}><i className="fa-solid fa-list" style={{ marginRight: "4px" }} />Address</span>,
      ...
      cell: (row) => (edit && rowid === row.id) ?
        <input
          ...
          autoComplete='off'
          autoFocus={currentEditField === 'Address'}
