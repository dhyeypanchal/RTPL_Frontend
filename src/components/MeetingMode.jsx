...
function MeetingMode() {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await instance.get("/api/meetingmode/get_meetingMode_list");
  ...
  useEffect(() => {
    const filterData = () => {
      const filter = data.filter((data) =>
        data.MeetingMode.toLowerCase().includes(searchQuery.toLowerCase())
      );
  ...
  const deleteMeetingModeData = async (cell) => {
    const token = localStorage.getItem("token")

    // axios request
    try {
      const req = await instance.delete(`/api/meetingmode/${cell.id}`, {
        headers: {
  ...
  const saveEdit = async (row) => {
    ...
    try {
      const req = await instance.put(`/api/meetingmode/${row.id}`,
        {
          meetingMode: editedData.MeetingMode,
          isActive: true
        }, {
        headers: {
          Authorization: `Bearer ${token}`
  ...
  const columns = [
    ...
    {
      name: <span style={{ fontSize: "125%" }}><i className="fa-solid fa-list" style={{ marginRight: "4px" }} />Meeting Mode</span>,
      selector: (row) => row.MeetingMode,
      cell: (row) => (edit && rowid === row.id) ?
        <input
          type="text"
          name="MeetingMode"
          className='form-control outline'
          value={editedData.MeetingMode}
          onChange={handleChange}
          autoComplete='off'
          autoFocus={currentEditField === 'MeetingMode'}
        /> : <span>{row?.MeetingMode}</span>,
      sortable: true,
      center: true,
      minWidth: "170px"
    },
        ...
        <span style={{ display: "flex", justifyContent: "space-between", width: "60px" }}>
          {(edit && rowid === row.id) ?
            ...
            <>
              ...
              <span
                style={{ cursor: "pointer" }}
                onClick={() => { deleteMeetingModeData(row) }}
              >
