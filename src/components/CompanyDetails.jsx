...
function CompanyDetails() {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await instance.get("/api/company/getCompanyList");
      // console.log(response.data.meetings);
      console.log(response.data.data);
      // setUsers(combined)
      // console.log(users);
      const totaldata = response.data.data.company.map((company) => ({
  ...
  const deleteCompanyData = async (cell) => {
    const token = localStorage.getItem("token")

    // axios request
    try {
      const req = await instance.delete(`/api/company/${cell.id}`, {
        headers: {
  ...
  const saveEdit = async (row) => {
    ...
    try {
      const req = await instance.put(`/api/company/${row.id}`,
        {
  ...
  const columns = [
    ...
    {
      name: <span style={{ fontSize: "125%" }}><i className="fa-solid fa-list" style={{ marginRight: "4px" }} />Name</span>,
      ...
      cell: (row) => (edit && rowid === row.id) ?
        <input
          ...
          autoComplete='off'
          autoFocus={currentEditField === 'Name'}
        /> : <span>{row?.Name}</span>,
      sortable: true,
      center: true,
      minWidth: "120px"
    },
    {
      ...
      cell: (row) => (edit && rowid === row.id) ?
        <input
          ...
          onChange={handleChange}
          autoComplete='off'
          autoFocus={currentEditField === 'Email'}
        /> : <span>{row?.Email}</span>,
      sortable: true,
      center: true,
      minWidth: "125px"
    ...
    {
      name: <span style={{ fontSize: "125%" }}><i className="fa-solid fa-list" style={{ marginRight: "4px" }} />Contact</span>,
      ...
      cell: (row) => (edit && rowid === row.id) ?
        <input
          ...
          onChange={handleChange}
          autoComplete='off'
          autoFocus={currentEditField === 'Contact'}
