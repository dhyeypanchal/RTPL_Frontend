...
function Roles() {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await instance.get("/api/role/getRoleList");
  ...
  const deleteRoleData = async (cell) => {
    const token = localStorage.getItem("token")

    // axios request
    try {
      const req = await instance.delete(`/api/role/${cell.id}`, {
        headers: {
  ...
  const saveEdit = async (row) => {
    ...
    try {
      const req = await instance.put(`/api/role/${row.id}`,
        {
  ...
  const columns = [
    ...
    {
      name: <span style={{ fontSize: "125%" }}><i className="fa-solid fa-list" style={{ marginRight: "4px" }} />Role</span>,
      ...
      cell: (row) => (edit && rowid === row.id) ?
        <input
          ...
          autoComplete='off'
          autoFocus={currentEditField === 'Role'}
        /> : <span>{row?.Role}</span>,
      sortable: true,
      center: true,
      minWidth: "170px"
